...
/** QIO redstone output component. */
public class BlockQIORedstoneAdapter extends BlockMekanismContainer {

    public BlockQIORedstoneAdapter() {
        super(Material.IRON);
        setHardness(3.5F);
        setResistance(8F);
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand,
        ...
        player.openGui(Mekanism.instance, QIOGuiConstants.REDSTONE_ADAPTER, world, pos.getX(), pos.getY(), pos.getZ());
        return true;
    }

    @Override
    public TileEntity createNewTileEntity(@Nonnull World world, int meta) {
        return new TileEntityQIORedstoneAdapter();
    }

    @Nonnull
    @Override
    protected ItemStack getDropItem(@Nonnull IBlockState state, @Nonnull IBlockAccess world, @Nonnull BlockPos pos) {
        ItemStack stack = new ItemStack(MekanismBlocks.QIO_REDSTONE_ADAPTER);
        TileEntity tile = world.getTileEntity(pos);
        if (tile instanceof TileEntityQIORedstoneAdapter adapter && stack.getItem() instanceof ItemBlockQIOComponent item) {
            item.setOwnerUUID(stack, adapter.getSecurity().getOwnerUUID());
            item.setSecurity(stack, adapter.getSecurity().getMode());
            if (adapter.getQIOFrequency() != null) {
                item.setFrequency(stack, adapter.getQIOFrequency().getIdentity());
            }
            item.setSustainedInventory(adapter.getInventory(), stack);
            item.setSustainedQIOData(adapter, stack);
        }
        return stack;
    }
    ...
    public int getWeakPower(IBlockState state, IBlockAccess world, BlockPos pos, EnumFacing side) {
        TileEntity tile = MekanismUtils.getTileEntitySafe(world, pos);
        return tile instanceof TileEntityQIORedstoneAdapter ? ((TileEntityQIORedstoneAdapter) tile).getRedstoneLevel() : 0;
    }
    ...
    public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess world, BlockPos pos) {
        return QIOBlockShapes.getBounds(QIOBlockShapes.REDSTONE_ADAPTER, state, world, pos);
    }
    ...
    public void addCollisionBoxToList(IBlockState state, @Nonnull World world, @Nonnull BlockPos pos,
        ...
        QIOBlockShapes.addCollisionBoxes(QIOBlockShapes.REDSTONE_ADAPTER, state, world, pos, entityBox, collidingBoxes);
    }
    ...
    public RayTraceResult collisionRayTrace(IBlockState state, @Nonnull World world, @Nonnull BlockPos pos,
        ...
        return QIOBlockShapes.collisionRayTrace(QIOBlockShapes.REDSTONE_ADAPTER, state, world, pos, start, end);
    }
