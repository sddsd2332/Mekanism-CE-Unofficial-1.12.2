...
/** QIO input automation component. */
public class BlockQIOImporter extends BlockMekanismContainer {

    public BlockQIOImporter() {
        super(Material.IRON);
        setHardness(3.5F);
        setResistance(8F);
    ...
    public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
        ...
        if (tile instanceof ISecurityTile security && placer instanceof EntityPlayer player && security.getSecurity().getOwnerUUID() == null) {
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand,
        ...
        player.openGui(Mekanism.instance, QIOGuiConstants.IMPORTER, world, pos.getX(), pos.getY(), pos.getZ());
        return true;
    }

    @Override
    public TileEntity createNewTileEntity(@Nonnull World world, int meta) {
        return new TileEntityQIOImporter();
    }

    @Nonnull
    @Override
    protected ItemStack getDropItem(@Nonnull IBlockState state, @Nonnull IBlockAccess world, @Nonnull BlockPos pos) {
        ItemStack stack = new ItemStack(MekanismBlocks.QIO_IMPORTER);
        TileEntity tile = world.getTileEntity(pos);
        if (tile instanceof TileEntityQIOImporter importer && stack.getItem() instanceof ItemBlockQIOComponent item) {
            item.setOwnerUUID(stack, importer.getSecurity().getOwnerUUID());
            item.setSecurity(stack, importer.getSecurity().getMode());
            if (importer.getQIOFrequency() != null) item.setFrequency(stack, importer.getQIOFrequency().getIdentity());
            item.setSustainedInventory(importer.getInventory(), stack);
            item.setSustainedQIOData(importer, stack);
        }
        return stack;
    }
    ...
    public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess world, BlockPos pos) {
        return QIOBlockShapes.getBounds(QIOBlockShapes.IMPORTER, state, world, pos);
    }
    ...
    public void addCollisionBoxToList(IBlockState state, @Nonnull World world, @Nonnull BlockPos pos,
        ...
        QIOBlockShapes.addCollisionBoxes(QIOBlockShapes.IMPORTER, state, world, pos, entityBox, collidingBoxes);
    }
    ...
    public RayTraceResult collisionRayTrace(IBlockState state, @Nonnull World world, @Nonnull BlockPos pos,
        ...
        return QIOBlockShapes.collisionRayTrace(QIOBlockShapes.IMPORTER, state, world, pos, start, end);
    }
