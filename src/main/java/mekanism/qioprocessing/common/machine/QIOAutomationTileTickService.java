    ...
    void registerChunk(@Nonnull Chunk chunk) {
        for (TileEntity tile : new ArrayList<>(chunk.getTileEntityMap().values())) {
            if (tile.isInvalid() || tile.getWorld() != chunk.getWorld() || chunk.getWorld().isRemote ||
                getRegisteredHost(tile) != null || QIOAutomationCapabilities.AUTOMATION_HOST == null) {
                continue;
            }
            try {
                if (!tile.hasCapability(QIOAutomationCapabilities.AUTOMATION_HOST, null)) {
                    continue;
                }
                QIOAutomationHost exposed = tile.getCapability(
                      QIOAutomationCapabilities.AUTOMATION_HOST, null);
                if (exposed instanceof DefaultQIOAutomationHost host && host.tile() == tile) {
