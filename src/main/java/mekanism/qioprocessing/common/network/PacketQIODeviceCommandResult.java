...
/**
 * QIO 处理模块中的 PacketQIODeviceCommandResult 类型。
 *
 * <p>该类型封装本层的数据、状态或服务职责；调用方应遵守其公开方法的输入约束，
 * 实现负责保持状态与持久化表示的一致。</p>
 */
public final class PacketQIODeviceCommandResult implements
      IMessageHandler<PacketQIODeviceCommandResult.Message, IMessage> {
    ...
    public IMessage onMessage(Message message, MessageContext context) {
        ...
        PacketHandler.handlePacket(() -> {
            ...
            if (state.matches(message.nonce, message.terminalUUID, message.targetRevision,
                ...
                container.getDeviceClientCache().applyCommand(message.nonce, message.result);
            }
        }, player);
        return null;
    }
    ...
    public static final class Message implements IMessage {
        ...
        private QIODeviceCommandResult result;
        ...
        public static Message create(int windowId, QIOProcessingTerminalSession session,
            ...
            return session == null || session.getFrequencyUUID() == null ? new Message() :
                  new Message(windowId, session, result);
        }
        ...
        public void toBytes(ByteBuf buffer) {
            ...
            PacketHandler.writeNBT(buffer, result.write());
        }
        ...
        public void fromBytes(ByteBuf buffer) {
            ...
            try {
                ...
                if (data == null) return;
                result = QIODeviceCommandResult.read(data);
                valid = shape();
